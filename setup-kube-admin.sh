...
KUBEADM_VERSION=${KUBEADM_VERSION:-1.31.0}
CRICTL_VERSION=${CRICTL_VERSION:-1.25.0}
RELEASE_VERSION=0.4.0 # TODO: Bump this to 0.18.0 (service links are missing)

pushd /usr/bin/
curl -L "https://github.com/kubernetes-sigs/cri-tools/releases/download/v${CRICTL_VERSION}/crictl-v${CRICTL_VERSION}-linux-amd64.tar.gz" | tar -C /usr/bin/ -xz
